...
public class CustomItem {
	...
	public CustomItem(@NotNull String identifierKey, @NotNull String displayName, int modelData,
			@NotNull WeaponMaterial material, @NotNull WeaponType type) throws InvalidMaterialException {
		...
		case IGNORED -> throw new InvalidMaterialException(
		...
		case WOOD, STONE, IRON, DIAMOND, GOLD, NETHERITE -> throw new InvalidMaterialException(
				"Tried to register item %s with invalid Material.".formatted(getDisplayName()));
		};
		...
		case SHIELD -> switch (material) {
		case IGNORED -> new ItemStack(Material.SHIELD, 1);
		case WOOD, STONE, IRON, DIAMOND, GOLD, NETHERITE -> throw new InvalidMaterialException(
				"Tried to register item %s with invalid Material.".formatted(getDisplayName()));
		};
		...
		final List<Component> lore = List.of(ColorParser.of("<gold>Custom Weapon").build()
